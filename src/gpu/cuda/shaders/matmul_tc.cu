...
// ---------------------------------------------------------------------------
// Q8 tile loader for tensor-core GEMM.
//
// Q8 block layout: 34 bytes per 32 weights (2-byte bf16 scale + 32 int8 values).
// Dequant: weight = float(q) * scale
// ---------------------------------------------------------------------------

static constexpr int Q8_BLOCK_SIZE_TC = 32;
static constexpr int Q8_BYTES_PER_BLOCK = 34;

__device__ __forceinline__ void load_weight_tile_q8(
    __nv_bfloat16 A_shared[TILE_N][TILE_K],
    ...
    const unsigned int blocks_per_row = K / Q8_BLOCK_SIZE_TC;
    const unsigned int k_block = k_start / Q8_BLOCK_SIZE_TC;
    ...
    for (unsigned int idx = tid; idx < total_elems; idx += 256) {
        ...
        __nv_bfloat16 val;
        if (global_row < M && (k_start + col) < K) {
            const unsigned char* row_data = W_q8 + global_row * blocks_per_row * Q8_BYTES_PER_BLOCK;
            const unsigned char* block_ptr = row_data + k_block * Q8_BYTES_PER_BLOCK;

            float scale = __bfloat162float(*((const __nv_bfloat16*)block_ptr));
            signed char q_val = (signed char)block_ptr[2 + col];
            val = __float2bfloat16((float)q_val * scale);
        } else {
            val = __float2bfloat16(0.0f);
        }
        A_shared[row][col] = val;
...
// Q8 tensor-core GEMM implementation.
// Identical to bf16/Q4 but uses Q8 tile loader.
__global__ void gemm_q8_tc_impl(
    const GemmParams params,
    const unsigned char* __restrict__ W_q8,
    const __nv_bfloat16* __restrict__ X,
    __nv_bfloat16* __restrict__ Y
    ...
    for (unsigned int kt = 0; kt < num_k_tiles; kt++) {
        ...
        load_weight_tile_q8(A_shared[buf], W_q8, tile_n_start, k_start, M, K, tid);
...
extern "C" __global__ void gemm_q8_tc(
    const GemmParams params,
    const unsigned char* __restrict__ W_q8,
    const __nv_bfloat16* __restrict__ X,
    __nv_bfloat16* __restrict__ Y
