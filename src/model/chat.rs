...
fn format_chatml(messages: &[Message], arch: ModelArch) -> String {
    ...
    //
    // Nemotron-H's HF template always emits a system block but with an empty
    // body when no system message is provided.  Injecting Qwen-specific text
    // shifts token positions and confuses the model.
