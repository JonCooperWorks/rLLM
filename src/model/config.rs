...
impl ModelConfig {
    ...
    pub fn recommended_kv_blocks(
        ...
        // Bytes per position for K and V pools (may differ in asymmetric mode).
        let k_bytes_per_pos = crate::model::turboquant::bytes_per_kv_position(
            self.head_dim, self.num_key_value_heads, kv_quant.k,
        ) as u64;
        let v_bytes_per_pos = crate::model::turboquant::bytes_per_kv_position(
            self.head_dim, self.num_key_value_heads, kv_quant.v,
        ) as u64;
        // bytes per block = (K + V) × num_layers × BLOCK_SIZE
        let bytes_per_block = self.num_hidden_layers as u64
            * kv_cache::BLOCK_SIZE as u64
            * (k_bytes_per_pos + v_bytes_per_pos);
