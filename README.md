...
| Qwen3.5 397B-A17B ⚡ | 397B (17B active) | **0.28 tok/s** | — | 3,500 ms | — |

⚡ = SSD expert streaming (`--stream-experts`).  Expert weights are read from NVMe on demand instead of loading all to GPU — the 397B model (751 GB on disk) runs on a 64 GB machine using ~20 GB GPU memory.  Parallel pread across K expert threads with fused gate+up reads saturates NVMe queue depth.  bf16 streaming is faster than Q4 because it skips CPU-side quantization — the bottleneck is NVMe bandwidth (~7 GB/s), not compute.
